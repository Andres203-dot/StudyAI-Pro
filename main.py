...
class StudyAIPro:
    ...
    def show_login(self):
        ...
        # Usuario
        tk.Label(frame, text="Usuario:", font=("Arial", 16), fg="white", bg="#2b2b2b").pack(pady=20)
        self.entry_user = tk.Entry(frame, font=("Arial", 14), width=30, justify="center")
        self.entry_user.pack(pady=10)
        self.entry_user.insert(0, "prueba")
        
        # Contraseña
        tk.Label(frame, text="Contraseña:", font=("Arial", 16), fg="white", bg="#2b2b2b").pack(pady=20)
        ...
        tk.Button(btn_frame, text="➕ REGISTRAR", font=("Arial", 16, "bold"), 
                 bg="#00cc88", fg="white", width=15, height=2,
                 command=self.register).pack(side="right", padx=20)
    ...
    def show_main(self):
        ...
        # Sidebar
        sidebar = tk.Frame(self.root, width=250, bg="#1e1e1e")
        sidebar.pack(side="left", fill="y", padx=(30, 15))
        sidebar.pack_propagate(False)
        
        tk.Label(sidebar, text="StudyAI Pro", font=("Arial", 24, "bold"), 
                fg="#00d4ff", bg="#1e1e1e").pack(pady=40)
        
        tk.Button(sidebar, text=" Dashboard", font=("Arial", 16), 
                 bg="#2b2b2b", fg="white", width=18, height=2,
    ...
    def show_dashboard(self):
        ...
        tk.Label(self.content, text=" Dashboard", font=("Arial", 32, "bold"), 
                fg="white", bg="#2b2b2b").pack(pady=50)
        ...
        stats_frame = tk.Frame(self.content, bg="#1f538d", relief="raised", bd=2)
        stats_frame.pack(pady=30, padx=50, fill="x")
        
        tk.Label(stats_frame, text=f"Sesiones: {stats[0] or 0}", 
                font=("Arial", 24, "bold"), fg="#00d4ff", bg="#1f538d").pack(pady=20)
        tk.Label(stats_frame, text=f"Horas: {(stats[1] or 0)/60:.1f}", 
                font=("Arial", 24, "bold"), fg="#ffaa00", bg="#1f538d").pack(pady=10)
        tk.Label(stats_frame, text=f"Promedio: {(stats[2] or 0):.1f}/10", 
                font=("Arial", 24, "bold"), fg="#00ff88", bg="#1f538d").pack(pady=20)
        
        tk.Label(self.content, text=" Tips: Haz sesiones de 25 min", 
                font=("Arial", 18), fg="#a0a0a0", bg="#2b2b2b").pack(pady=30)
    
    def show_new_session(self):
        for w in self.content.winfo_children(): w.destroy()
        
        tk.Label(self.content, text="➕ Nueva Sesión", 
                font=("Arial", 32, "bold"), fg="white", bg="#2b2b2b").pack(pady=60)
        
        frame = tk.Frame(self.content, bg="#2b2b2b")
        frame.pack(pady=40)
        
        tk.Label(frame, text="Materia:", font=("Arial", 18), fg="white", bg="#2b2b2b").pack(pady=20)
        self.new_subject = tk.Entry(frame, font=("Arial", 16), width=40)
        self.new_subject.pack(pady=10)
        
        tk.Label(frame, text="Minutos:", font=("Arial", 18), fg="white", bg="#2b2b2b").pack(pady=20)
        self.new_duration = tk.Entry(frame, font=("Arial", 16), width=40)
        self.new_duration.insert(0, "25")
        self.new_duration.pack(pady=10)
        
        tk.Label(frame, text="Puntuación (1-10):", font=("Arial", 18), fg="white", bg="#2b2b2b").pack(pady=20)
        self.new_score = tk.Entry(frame, font=("Arial", 16), width=40)
        self.new_score.insert(0, "8")
        self.new_score.pack(pady=10)
        
        tk.Button(frame, text=" GUARDAR", font=("Arial", 18, "bold"), 
                 bg="#00cc88", fg="white", width=20, height=2,
                 command=self.save_session).pack(pady=40)
    ...
    def show_stats(self):
        ...
        for subj, hours, count, avg in c.fetchall():
            ...
            tk.Label(frame, text=text, font=("Arial", 16), fg="white", bg="#2b2b2b",
                    relief="ridge", padx=20, pady=15).pack(fill="x", pady=10)
